...
export const RecruiterRadarView: React.FC<RecruiterRadarViewProps> = ({
  ...
  const activeRecruiter = useMemo(() => {
    ...
    return recruiters.find(r => r.id === selectedRecruiterId) || filteredRecruiters[0] || recruiters[0] || null;
  }, [recruiters, selectedRecruiterId, filteredRecruiters]);
  ...
  const handleResetCanonical = () => {
    const list = RecruiterRadarService.resetToCanonical();
    setRecruiters(list);
    if (list.length > 0) {
  ...
  const handleAddRecruiterSubmit = (e: React.FormEvent) => {
    ...
    const added = RecruiterRadarService.addRecruiter({
      ...
      verifiedEmail: newEmail.trim() || `${newFullName.toLowerCase().replace(/\s+/g, '.')}@${newCompanyName.toLowerCase().replace(/\s+/g, '')}.com`,
      linkedinUrl: newLinkedin.trim() || `https://linkedin.com/in/${newFullName.toLowerCase().replace(/\s+/g, '-')}`,
    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-6 space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-xl">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          <div className="p-2.5 px-3 rounded-xl bg-slate-950 border border-slate-800 text-center hidden sm:block">
            <div className="text-[10px] font-mono text-slate-400 uppercase">Avg Response Prob</div>
            <div className="text-base font-bold font-mono text-cyan-300">{stats.avgResponseRatePercent}% Rate</div>
          </div>
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono px-2 py-0.2 rounded bg-slate-800 text-slate-300">
                  {selectedOpp.verification?.requisitionId || 'VERIFIED-REQ'}
                </span>
              </div>
            ...
            >
              {opportunities.map(opp => (
                <option key={opp.id} value={opp.id}>
                  {opp.companyName} — {opp.title.slice(0, 30)}...
                </option>
              ))}
            </select>
            {onOpenOpportunity && (
      ...
      {/* ZERO CONTACTS EMPTY STATE: RENDERED IF RECRUITERS ARRAY IS EMPTY */}
      {recruiters.length === 0 ? (
        <div className="p-12 rounded-2xl bg-slate-900 border border-slate-800 text-center space-y-5">
          <div className="w-14 h-14 rounded-2xl bg-purple-950/50 border border-purple-800 flex items-center justify-center mx-auto text-purple-400">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-5 space-y-3">
                  ...
                  <div className="space-y-3 max-h-[640px] overflow-y-auto pr-1">
                    {filteredRecruiters.length === 0 ? (
                      <div className="p-8 rounded-2xl bg-slate-900 border border-slate-800 text-center text-slate-400 text-xs font-mono space-y-3">
                        <p>No recruiter contacts match your filter criteria.</p>
                        <button
                          onClick={() => {
                            setSearchQuery('');
                            setRoleFilter('all');
                          }}
                          className="px-3 py-1.5 rounded-lg bg-purple-600/30 text-purple-300 border border-purple-500/40 text-xs"
                        >
                          Reset Filters
                        </button>
                      </div>
                ...
                <div className="lg:col-span-7 space-y-4">
                  {activeRecruiter && outreachPackage ? (
                    ...
                    <div className="p-8 rounded-2xl bg-slate-900 border border-slate-800 text-center text-slate-400 font-mono text-xs space-y-3">
                      <Users className="w-8 h-8 text-purple-400 mx-auto" />
                      <p>Select a recruiter contact on the left to inspect their personalized outreach package.</p>
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-6">
              {activeRecruiter ? (
                ...
                <div className="p-8 rounded-2xl bg-slate-900 border border-slate-800 text-center text-slate-400 font-mono text-xs">
                  No recruiter selected. Please select or add a verified contact.
                </div>
              )}
            </div>
          )}
              ...
              {activeRecruiter && outreachPackage ? (
                <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      Statistically optimal follow-up timeline for {activeRecruiter.fullName} to maximize interview conversions without crossing etiquette boundaries
                    </p>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="w-full max-w-lg rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl p-6 space-y-5">
            ...
            <form onSubmit={handleAddRecruiterSubmit} className="space-y-4 text-xs font-mono">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    <option value="Principal Technical Recruiter">Principal Technical Recruiter</option>
              ...
              <div className="space-y-1">
                <label className="text-slate-300">Department / Focus:</label>
                <input
                  type="text"
                  placeholder="e.g. Infrastructure, Distributed Systems"
                  value={newDepartment}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-slate-300">Verified Email (Optional):</label>
                  <input
                    type="email"
                ...
                <div className="space-y-1">
                  <label className="text-slate-300">LinkedIn URL (Optional):</label>
                  <input
                    type="url"
