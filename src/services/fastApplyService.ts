...
export class FastApplyService {
  ...
  public static generateConfirmationId(companyName: string): string {
    const prefix = 'CONF-' + new Date().getFullYear();
  ...
  public static async executeSafeSubmission(
    ...
    if (onProgressUpdate) {
      onProgressUpdate({
        ...
        detail: `Mapping ${profile.fullName} • ${profile.email} • ${profile.collegeName} (${profile.degree})`,
        completed: false,
      });
    }
    ...
    // Save receipt in permanent dossier
    this.saveReceipt(receipt);

    // Auto-update Kanban stage in RadarEngine to 'applied' (Req #3)
    ...
    RadarEngine.generateSimulatedEmail({
      ...
      tier: 'slate',
      subject: `[CONFIRMED ${confirmationId}] Official Submission Receipt: ${opportunity.title} @ ${opportunity.companyName}`,
    ...
    if (onProgressUpdate) {
      onProgressUpdate({
        step: 5,
        title: 'Application Confirmed & Certified',
        detail: `Official Confirmation ID: ${confirmationId} • Kanban pipeline updated to Applied.`,
        completed: true,
      });
    }
